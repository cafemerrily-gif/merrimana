...
const navItems = [
  ...
  { href: "/accounting", icon: BarChart2, label: "会計" },
  { href: "/products", icon: ShoppingBag, label: "商品" },
  { href: "/marketing", icon: Megaphone, label: "広報" },
  { href: "/system", icon: Settings, label: "システム" },
  { href: "/store", icon: Store, label: "店舗" },
];
...
> = {
  ...
  "/marketing": [
    ...
    { href: "/marketing/sns", icon: Share2, label: "SNS管理" },
    { href: "/marketing/media", icon: Image, label: "メディア素材" },
  ],
  "/system": [
    { href: "/system", icon: Users, label: "ユーザー管理" },
    { href: "/system/roles", icon: ShieldCheck, label: "ロール・権限" },
    { href: "/system/master", icon: Database, label: "マスタ設定" },
  ],
  "/store": [
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const subNav = activeSection ? sectionSubNav[activeSection] : null;
  ...
  return (
    <>
      <div className="flex h-dvh overflow-hidden bg-white dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <div className="flex flex-1 min-h-0">
            {/* 中央ペイン：メインコンテンツ（モバイルは下ナビ分 pb-20） */}
      ...
      {/* ── 下ナビ（スマホ・iPad: lg未満のみ） ── */}
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 dark:bg-neutral-950/95 backdrop-blur border-t border-neutral-200 dark:border-neutral-800 flex items-stretch"
...
function RightPanel({ section }: { section: string | null }) {
  if (!section) {
    return (
      <div className="space-y-2">
        <QuickLink href="/accounting" label="売上を確認" />
        <QuickLink href="/store" label="日報を入力" />
        <QuickLink href="/products" label="商品を管理" />
      </div>
    );
  }
  const actions: Record<string, { label: string; href: string }[]> = {
    ...
    "/marketing": [
      ...
      { label: "SNS投稿を予約", href: "/marketing/sns" },
      { label: "素材をアップロード", href: "/marketing/media" },
    ],
    "/system": [
      { label: "ユーザーを招待", href: "/system" },
      { label: "権限を変更", href: "/system/roles" },
    ],
    "/store": [
